 ...
 * `get_report_instructions` tool) and the `GET /api/admin/ai-reports/instructions`
 * endpoint, so the guidance an agent receives never drifts between channels.
 *
 * Pure (no DB / IO) so it is trivial to test and reuse.
 */

const AVAILABLE_R_PACKAGES = ['RDS', 'tidyverse', 'lubridate', 'scales', 'jsonlite', 'uuid'];
...
function getReportInstructions(ctx = {}) {
    ...
    return `# SALT Quarto report builder — instructions

You are an expert biostatistician building a **Quarto (.qmd) report in R** for ${target}.
...
- Only these R packages are installed — do not \`library()\` anything else:
  ${AVAILABLE_R_PACKAGES.map(p => '`' + p + '`').join(', ')}.
...
module.exports = { getReportInstructions, AVAILABLE_R_PACKAGES };
