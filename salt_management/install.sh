...
#   --enable-mcp        Enable the MCP report-builder server (off by default).
#                       Sets MCP_ENABLED=true. MCP_PUBLIC_URL is set to
#                       https://<domain> regardless, so it's correct if you
#                       enable MCP later by editing the compose file.
...
log "MCP report builder: $([[ "$ENABLE_MCP" -eq 1 ]] && echo "ENABLED" || echo "disabled (pass --enable-mcp to turn on)"); public URL https://$DOMAIN"
...
AI report builder: $( [[ "$ENABLE_MCP" -eq 1 ]] && echo "ENABLED — connect a client to https://$DOMAIN/mcp" || echo "disabled — set MCP_ENABLED: \"true\" in $COMPOSE_FILE (or re-run with --enable-mcp) to turn on" )
