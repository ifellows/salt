...
Setup is done with the tablet **connected to the internet**:

1. **Install the app** — on the tablet, browse to `https://<your-server>/tablet` and follow the install instructions on that page.
2. **Get the facility set-up code** from your administrator.
3. **Open the SALT app** on the tablet and follow the on-screen instructions, entering the facility set-up code when prompted.
4. **Log in as an administrator** and create a new **Staff** user.
5. **Log in as that Staff user** to start performing surveys.
