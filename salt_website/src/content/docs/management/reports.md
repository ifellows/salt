...
The report code has access to the SALT SQLite database via `DBI` and `RSQLite`. Packages including `RDS`, `tidyverse`, `lubridate`, `scales`, `jsonlite`, and `uuid` are available.
